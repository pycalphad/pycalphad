...
Unless you already have them, install VSCode (or other IDE for Python), Git, C/C++ compilers, and a suitable version of Python.

Windows users should install Build Tools for Visual Studio version 14.X.
...
After completing those downloads, `click <https://marketplace.visualstudio.com/items?itemName=ms-python.python>`_ to download Microsoft's Python extension for VSCode.

We strongly reccomend using uv as your Python package manager.
Besides being fast, uv automatically manages your Python installation, virtual environments, and keeps packages in sync during development, following best practices (`<https://docs.astral.sh/uv/pip/environments/>`_).
You can follow the installation instructions on Astral website: `uv installation <https://docs.astral.sh/uv/getting-started/installation/>`_.

Now, you can install the latest development branch of pycalphad. Run this command in a shell (like bash):
...
#. Run ``uv run pytest`` in your terminal to check your code changes locally.
...
To build a local copy of the pycalphad website using the code repository, follow the instructions below.
This is helpful for when you write a pull request because you want to update the documentation at the same time that you add a new feature.
This way you can test changes to the documentation without having to push them to GitHub.
...
- If you are still having issues, try searching for the error message online or asking for help on the `pycalphad GitHub repository <https://github.com/pycalphad/pycalphad/discussions>`_ or in the `pycalphad Gitter channel <https://gitter.im/pycalphad/pycalphad>`_.
