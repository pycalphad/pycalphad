...
def halton(dim, nbpts, primes=None, scramble=True):
    ...
    result = np.empty((nbpts, dim), dtype=float)
    dim_primes = np.asarray(primes[0:dim], dtype=float)
    for i in np.arange(dim_primes.size):
        num_powers = np.ceil(np.log(nbpts + 1) / np.log(dim_primes[i])).astype(int)
        ...
        sum_matrix = np.outer(np.arange(1, nbpts+1), radix_vector).astype(float)
