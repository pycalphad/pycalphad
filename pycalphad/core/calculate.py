...
def _compute_phase_values(components, statevar_dict, str_phase_local_conditions,
    ...
    if points.size > 0:
        ...
        if parameter_array_length <= 1:
            phase_output = np.reshape(np.asarray(phase_output, dtype=np.float64), shape=points.shape[:-1])
        else:
            phase_output = np.reshape(np.asarray(phase_output, dtype=np.float64), shape=points.shape[:-1] + (parameter_array_length,))
        phase_compositions = np.reshape(np.asarray(phase_compositions, dtype=np.float64), shape=points.shape[:-1] + (len(pure_elements),))
    else:
