...
def starting_point(conditions, state_variables, phase_records, grid):
    ...
    coord_dict['vertex'] = np.arange(len(nonvacant_elements))
    coord_dict['component'] = nonvacant_elements
    conds_as_strings = [str(k) for k in conditions.keys()]
    if cond_analysis.global_min:
        result = Dataset({'NP':     (conds_as_strings + ['vertex'], np.empty(grid_shape + (len(nonvacant_elements),))),
                          ...
                          'X':      (conds_as_strings + ['vertex', 'component'],
                                     np.empty(grid_shape + (len(nonvacant_elements), len(nonvacant_elements),))),
                          'Y':      (conds_as_strings + ['vertex', 'internal_dof'],
                                     np.empty(grid_shape + (len(nonvacant_elements), maximum_internal_dof,))),
                          'Phase':  (conds_as_strings + ['vertex'],
                                     np.empty(grid_shape + (len(nonvacant_elements),), dtype='U%s' % max_phase_name_len)),
                          'points': (conds_as_strings + ['vertex'],
                                     np.empty(grid_shape + (len(nonvacant_elements),), dtype=np.int32))
                          },
