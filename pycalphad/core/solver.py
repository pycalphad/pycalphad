...
class InteriorPointSolver(object):
    ...
    def solve(self, prob):
        ...
        nlp.addOption(b'tol', 1e-12)
        nlp.addOption(b'acceptable_tol', 1e-11)
        nlp.addOption(b'acceptable_constr_viol_tol', 1e-11)
        nlp.addOption(b'constr_viol_tol', 1e-12)
        nlp.addOption(b'bound_relax_factor', 1e-16)
        ...
        print('prod', np.dot(prob.jacobian(x)[-len(set(comps) - {'VA'}):], info['mult_x_L']))
